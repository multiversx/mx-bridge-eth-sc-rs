deployFaucet() {
    ...
    mxpy contract deploy --bytecode=${FAUCET_WASM} "${MXPY_SIGN[@]}" \
    --gas-limit=20000000 \
...
mintAndDeposit() {
  ...
  mxpy contract call ${ALICE_ADDRESS} "${MXPY_SIGN[@]}" \
  ...
  mxpy contract call ${FAUCET} "${MXPY_SIGN[@]}" \
...
deployTestCaller() {
    ...
    mxpy contract deploy --bytecode=${TEST_CALLER_WASM} "${MXPY_SIGN[@]}" \
    --gas-limit=20000000 \
