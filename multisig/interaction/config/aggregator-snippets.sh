deployAggregator() {
    ...
    mxpy contract deploy --bytecode=${AGGREGATOR_WASM} "${MXPY_SIGN[@]}" \
...
stakeOracles() {
    ...
    STAKE=$(echo "$ORACLE_REQUIRED_STAKE*10^18" | bc)
    echo "---------------------------------------------------------"
    mxpy --verbose contract call ${AGGREGATOR}   --pem=${ORACLE_WALLET0} \
    --gas-limit=35000000 --function="stake" --value=${STAKE} \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
    echo "---------------------------------------------------------"
    echo "---------------------------------------------------------"
    mxpy --verbose contract call ${AGGREGATOR}   --pem=${ORACLE_WALLET1} \
    --gas-limit=35000000 --function="stake" --value=${STAKE} \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
    echo "---------------------------------------------------------"
    echo "---------------------------------------------------------"
    mxpy --verbose contract call ${AGGREGATOR}   --pem=${ORACLE_WALLET2} \
    --gas-limit=35000000 --function="stake" --value=${STAKE} \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
    echo "---------------------------------------------------------"
}
...
setPairDecimals() {
    ...
    mxpy contract call ${AGGREGATOR} "${MXPY_SIGN[@]}" \
...
pauseAggregatorV2() {
    ...
    mxpy contract call ${AGGREGATOR_v2} "${MXPY_SIGN[@]}" \
    --gas-limit=5000000 --function="pause" \
    --send --proxy=${PROXY} --chain=${CHAIN_ID} || return
}
...
aggregator-upgrade() {
    ...
    mxpy contract upgrade ${AGGREGATOR} --bytecode=${AGGREGATOR_WASM} "${MXPY_SIGN[@]}" \
